...
#define Sn_IR_CON					0x01		/**< established connection */
	...
	{
		IINCHIP_WRITE(Sn_CR(s),Sn_CR_LISTEN); //Sn_CR_OPEN
		/* +20071122[chungs]:wait to process the command... */
		Delay_ms(100);
		while( IINCHIP_READ(Sn_CR(s)) ) 
			;
...
#ifdef __DEF_IINCHIP_INT__
	{while ( (getISR(s) & Sn_IR_SEND_OK) != Sn_IR_SEND_OK )CH0_Putchar('2'); }
 
#else
	{while ( (IINCHIP_READ(Sn_IR(s)) & Sn_IR_SEND_OK) != Sn_IR_SEND_OK ) CH0_Putchar('3');}
...
//+2008.01 bj 	
#ifdef __DEF_IINCHIP_INT__
  	putISR(s, getISR(s) & (~Sn_IR_SEND_OK)); CH0_Putchar('6');
#else
	IINCHIP_WRITE(Sn_IR(s), Sn_IR_SEND_OK);	CH0_Putchar('7');
#endif
 	CH0_Putchar('8');
  */
  	return ret;
